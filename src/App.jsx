...
Génère le code complet pour répondre à cette description.`;
  ...
  const generateArtifact = (generatedCode) => {
  ...
  const artifactToXml = (artifact) => {
    let xml = '<div class="__boltArtifact__" data-message-id="x8Hl5WIatF2RL3oO"></div>`;
    }
  ...
  const startWebContainer = async (artifact) => {
    try {
      ...
      if (artifact.commands && artifact.commands.length > 0) {
        for (const command of artifact.commands) {
          await webcontainer.exec({ command: command.split(' ')[0], args: command.split(' ').slice(1) });
        }
      }

      // 5. Start the development server
      const process = await webcontainer.exec({ command: 'npm', args: ['run', 'dev'] });
